...
public class Parameter {
    ...
    public Parameter(String name, Object... values) {//varargs ...
        this.name = name;
        setValues(values);
    }

    public static Optional<Parameter> find(List<Parameter> params, String paramName) {
        return params.stream().filter(p -> p.getName().equals(paramName)).findFirst();//The filter method is used to select elements as per the Predicate passed as argument.
        //find first is used to return the first element of the stream.
    }
