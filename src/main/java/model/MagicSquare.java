...
public class MagicSquare extends CSP<Variable,Integer> {
	...
	public MagicSquare(int n) {
		...
		float magic = ((1f / 2f) * (n) * (n * n + 1));//Magic constant
		this.sum = (int) magic;
	...
	public void solve() {
		...
		Optional<Assignment> solution = solver.solve(this.copyDomains());

		if (solution != null) {
			showSquare(solution.get()  );
		} else {
	...
	public void showSquare(Assignment solution) {
		for (int i = 0; i < getVariables().size(); i++) {
			if (i % dimension == 0) {
				System.out.print("\n");
			}
			System.out.print("  "
					+ solution.getValue(getVariables().get(i)));
		}
		System.out.println("\n");
	}
