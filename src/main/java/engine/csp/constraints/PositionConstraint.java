...
/**
 * Implementation of Position Constraint constraint.
	...
	public boolean isSatisfiedWith(Assignment assignment) {
		...
		if (assignment.isComplete(variables)) {
			...
			for (int j = 0; j < variables.size() ; j++) {
				...
				value = assignment.getValue(variables.get(j));
				...
				if (index - (distance + 1) >= 0) {
					if (value == assignment.getValue(variables.get(index - (distance + 1)))){
		...
		for (int i = 0; i < variables.size(); i++) {
			value = assignment.getValue(variables.get(i));
			index = getVarIndex(variables.get(i));
			distance = getDistance(value);

			for (int j = 1; j <= distance; j++) {
				if ((index + j) < variables.size()) {
					if (value == assignment.getValue(variables.get(index
							+ j))) {
						return false;
					}
				}
